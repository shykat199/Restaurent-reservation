                        ...
                        <div class="mt-1">
                            <select id="status" name="status" class="form-multiselect block w-full mt-1">
                                @foreach (App\Enums\TableStatus::cases() as $status)
                                    <option
                                        @selected($table->status===$status ? 'selected':'') value="{{ $status->value }}">{{ $status->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('status')
                        ...
                        <div class="mt-1">
                            <select id="location" name="location" class="form-multiselect block w-full mt-1">
                                @foreach (App\Enums\TableLocation::cases() as $location)
                                    <option
                                        @selected($table->location===$location ? 'selected':'') value="{{ $location->value }}">{{ $location->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('location')
