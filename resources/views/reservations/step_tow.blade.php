                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                        <div class="w-full">
                            ...
                            <form method="POST" action="{{route('reservations.store.step.tow')}}">
                                @csrf
                                    ...
                                    <div class="mt-1">
                                        ...
                                        <select id="table_id" name="table_id" class="form-multiselect block w-full mt-1">
                                            @foreach ($tables as $table)
                                                <option
                                                    value="{{ $table->id }}" @selected($table->id === $reservation->table_id)>
                                                    {{ $table->name }}
                                                    ({{ $table->guest_number }} Guests) &nbsp; {{$table->location}}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
